...
int main(int argc, char **argv)
{
    ...
    if (!(size == 4))
    {
        ...
        MPI_Finalize();   
        throw std::runtime_error("Число MPI процессов не 4!");
    }
    ...
    N_ofDomen = (std::ceil((Ns + 1) / 2) + 1); // длина = 46 (нумеруя с нуля)
    ...
    for (int k = 1; k < k_max; k++)
    {
        ...
        get_matrixA_OMP((const double**)(a_CoeffMatrix + startY_Indx),
                (const double**)(b_CoeffMatrix + startY_Indx),
                startX_Indx, (const double**)r_k_loc,
                h1, h2, N_ofDomen, M_ofDomen, Ar_loc);
